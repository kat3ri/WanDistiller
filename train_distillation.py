...
def setup_distributed():
    ...
    try:
        ...
        print(file=sys.stderr)
        print("  2. Try using gloo backend for CPU/testing:", file=sys.stderr)
        print("     (requires code modification to change backend)", file=sys.stderr)
        print(file=sys.stderr)
        print("  3. Ensure proper network configuration if using multiple nodes", file=sys.stderr)
        print(file=sys.stderr)
        print("  4. Check firewall settings if communication fails", file=sys.stderr)
        print(file=sys.stderr)
        print("=" * 80, file=sys.stderr)
        sys.exit(1)
